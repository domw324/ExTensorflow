...
## 1. Anaconda 설치
- 아나콘다는 ~
- 가상환경 설치 시, python 버전을 꼭 잘 확인하자. 현재 집 PC와 회사 PC의 가상환경 설정이 달라 코드가 제대로 돌아가지 않는다...
## 2. Tensorflow 설치
...
## 다층 퍼셉트론 레이어 (MLP, Multi Layer Perceptron)
...
## 심층 신경망
- 더 어렵고 진화된 데이터세트에서의 분류 작업의 정확도 측면에서 탁월한 성능을 보인다.

### 함수형 API (Functional API)
케라스에서 신경망을 구축하는 또 다른 방법. 순차형 모델(Sequential Model)로는 구축할 수 없는 더 복잡한 신경망을 구축할 수 있게 한다.
> 함수형 API에 대한 자세한 내용은 https://keras.io/ 에서 확인

##### 순차형 모델의 한계
1. 순차형 모델은 하나의 계층이 다른 계층 위에 쌓이는 방식이다. 일반적으로 입력과 출력 계층을 통해 이 모델에 접속한다. 그래서
신경망 중간에 보조 입력을 추가하거나 최종 계층 전에 보조 출력을 추출해야 할 때 이를 해결할 간단한 방법이 없다.
2. 그래프 같은 모델이나 파이썬 함수처럼 동작하는 모델을 지원하지 않음
3. 두 모델 사이에 계층을 공유하기 어렵다.

위와 같은 한계는 **함수형 API**로 해결할수 있다!

##### 기본 개념
- 계층은 텐서를 인수로 받는 인스턴스다. 한 계층의 출력은 또 다른 텐서다. 모델을 구성하는 계층 인스턴스는 입력과 출력 텐서를 통해 서로 연결되는 객체다. 이렇게 하면 순차형 모델에서 여러 계층을 쌓아 올리는 것과 비슷한 최종 결과를 얻게 된다. 그렇지만 계층 인스턴스를 사용하면 각 계층의 입출력에 손쉽게 접근할 수 있기 때문에 모델이 보조 입출력 혹은 여러 입출력을 갖기 쉽다.
...
### 심층 잔차 신경망(ResNet)
잔차 학습 개념을 도입, 심층 합성곱 신경망에서 경사 소실(vanishing gradient) 문제를 해결함으로써 매우 깊은 신경망을 구축할 수 있게 한다.

##### 주요 이점
- 입력과 특징 맵에서 다양한 수준의 표현을 학습하는 데 탁월
- 분류(Classification), 분할(Segmentation), 탐지(Detetion) 등 다양한 컴퓨터 비전 문제에서 다양한 수준의 특징을 학습할 수 있으면 일반적으로 더 나은 성능을 이끌어 낼 수 있다.

##### 맹점
- 역전파하는 동안 얕은 계층에서 깊이에 따라 경사가 소실(gradient vanishing)되거나 발산(gradient exploding)되기 때문에 심층 신경망을 훈련시키기 쉽지 않다.
- 역전파는 연쇄 법칙을 기반하므로 경사는 얕은 계층에 도달할 때 줄어드는 경향이 있다.
- 특히 오차와 매개변수의 절댓값이 매우 작을 경우, 거기에 작은 수를 곱하기 때문에 생기는 현상이다.
- 경사가 줄어들면 매개변수가 적절히 업데이트 되지 않아, 네트워크 성능 개선이 제대로 이루어지지 않는다.

#### 심층 잔차 학습 프레임워크
: 심층 신경망에서 경사가 줄어드는 것을 완화하기 위해 도입. 핵심은 **숏컷 연결(shortcut connection)**


### 밀집 연결 합성곱 네트워크(DenseNet)
숏컷 연걸을 사용하는 대신, 이전 특징맵 전체가 다음 계층의 입력이 된다.
- 모든 합성곱에서 입력과 더 낮은 계층의 특징 맵에 직접 접근할 수 있게 함으로써 ResNet 기법을 개선
- 네트워크가 깊어질수록, 특징 맵이 급속도로 커지기 때문에 계산 속도거 느려지거나, 특징 맵 크기가 줄어들면서 커널의 범위가 늘어나게 된다.
- 병목(Bottleneck)과 전이(Transition) 계층을 모두 활용함으로써 심층 신경망에서 매개변수 개수를 낮게 유지한다.
  - 병목 계층은 특징 맵의 개수가 비효율적인 지점까지 증가하는 것을 막는 역할
  - 전이 계층은 두 밀집 블록 사이에서 특징맵의 크기를 작게 바꿔주는 역할